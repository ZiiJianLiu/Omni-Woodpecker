...
DEFAULT_MANIFEST = (
    ...
    / "10_target_typed_manifests"
    / "unary_rebalance_manifest.jsonl"
)
DEFAULT_POLICY_ROWS = (
    ROOT
    / "results"
    / "unary_full_minus_target_raw_prior_beta0p65_alpha0p00_full898_20260606"
    / "adaptive_conflict_bottleneck"
    / "30_raw_policy_by_layer_group"
    / "adaptive_conflict_bottleneck__base_24w0p23__26w0p61__27w0p16"
    / "rows.jsonl"
)
DEFAULT_OUTPUT_DIR = ROOT / "results" / "videollama2_lccs_cross_model_avh300_20260609"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cross-model LCCS validation on VideoLLaMA2-AV.")
    ...
    parser.add_argument("--selection-mode", choices=["qwen_edit_applied", "avh_unary_full"], default="qwen_edit_applied")
...
def main() -> None:
    ...
    run_config = {
        "kind": "videollama2_lccs_cross_model_v1",
    ...
    iterator = tqdm(selected, desc=f"videollama2_lccs_s{args.shard_index}", disable=bool(args.no_progress), unit="sample")
    for row in iterator:
        start = time.time()
        sample_id = safe_text(row.get("sample_id"))
