...
DEFAULT_MANIFEST = (
    ROOT
    / "results/prior_carrier_suppression_manifest_20260603/prior_carrier_suppression_manifest.jsonl"
)
DEFAULT_OUTPUT_DIR = ROOT / "results/prior_carrier_coupling_audit_20260603"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Capture-only audit for evidence-preserved prior-carrier suppression. "
...
def runtime_rows(args: argparse.Namespace, manifest_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    if explicit_runtime_rows is not None:
        ...
        for manifest in manifest_rows:
            role = safe_text(manifest.get("manifest_role"))
            ...
            row.update(
                {
                    ...
                    "current_answer_y0": normalize_binary_label(manifest.get("current_answer_y0")),
                    "candidate_answer_y": normalize_binary_label(manifest.get("candidate_answer_y")),
                    "evidence_certificate": safe_text(manifest.get("evidence_certificate")),
                    "current_best_accepts": bool(manifest.get("current_best_accepts")),
                }
            )
            if keep_offline:
                reference = (
...
def run() -> None:
    ...
    if args.plan_only:
        print(f"[prior-carrier-audit] plan-only selected={len(selected)} output_dir={args.output_dir}")
        return
    ...
    summary = {
        "kind": "prior_carrier_coupling_audit_v1",
